...
консольный ввод делать не надо, список фильмов можно инциализировать самим в коде*/
    public static void main(String[] args) {
        ...
        listResult = new ArrayList<>(list1);
        ...
        listResult = list1.stream().filter(list2::contains).collect(Collectors.toList());
        System.out.println("\nЗадание 2.");
        ...
        for (Film item : list1) {
            ...
            for (Film film : list2) {
                if (item == film) {
                    listResultWithoutStream.add(item);
                }
            }
        ...
        listResultWithoutStream.addAll(list2);
        ...
        listResult = listResult.stream().sorted(Film::compareTo).distinct().collect(Collectors.toList());
        System.out.println("\nЗадание 4.");
        ...
        long count = list1.stream().filter((p) -> p.getRang() == 7.5).count();
        ...
        //Реализация без использования Stream
        for (Film film : list1) {
            if (film.getRang() >= maxRang) {
                listResultWithoutStream.add(film);
            }
        }
        ...
        for (Film film : list1) {
            String currentName = film.getName();
        ...
        for (Film film : list1) {
            double currentRang = film.getRang();
        ...
        for (Film film : list1) {
            ...
            if (currentRang > maxy) {
                maxy = currentRang;
                filmWithMaxRang = film;
            }
            if (currentRang < miny) {
                miny = currentRang;
                filmWithMinRang = film;
            }
        }
